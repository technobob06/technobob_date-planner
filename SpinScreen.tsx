  ...
  return (
    <div className={`h-16 w-full flex items-center bg-white relative group ${locked ? 'bg-gray-50' : ''}`}>
      ...
      <div className="flex-1 overflow-hidden h-full relative ml-6">
        ...
        >
          {/* 这里通过组合数组确保有足够的滚动内容，使用索引作为辅助 key */}
          {[...items, ...items, ...items, ...items, ...items, ...items].map((item: DateOption, idx: number) => (
  ...
  return (
    <div className="flex-1 flex flex-col p-8 bg-[#fdfbf7] animate-fadeIn">
      ...
      <button 
        onClick={handleSpin}
        disabled={spinning}
        className={`w-full py-5 rounded-full text-lg font-bold shadow-2xl transition-all ${
          spinning ? 'bg-gray-100 text-gray-300' : 'bg-[#e5989b] text-white'
        }`}
      >
        {spinning ? 'Төлөвлөж байна...' : '✨ Эргүүлэх'}
      </button>
    </div>
  );
};
